...
#import "SYZStickyPageController.h"
#import "SYZMagicScrollView.h"

@interface SYZStickyPageController () <SYZMagicScrollViewDelegate>

@property(nonatomic, strong) SYZMagicScrollView *contentView;

@end

@implementation SYZStickyPageController
@dynamic delegate;
...
//    [super viewDidLayoutSubviews];
    if ( self.contentView.contentOffset.y < self.maximumHeaderViewHeight - self.minimumHeaderViewHeight) {
        [super viewDidLayoutSubviews];
    }
...
#pragma mark - SYZMagicScrollViewDelegate

- (BOOL)scrollView:(SYZMagicScrollView *)scrollView shouldScrollWithSubview:(UIScrollView *)subview {
    if ([subview isKindOfClass:WMScrollView.class]) {
        return NO;
    }
    
    if ([self.delegate conformsToProtocol:@protocol(SYZStickyPageControllerDelegate)]) {
        id<SYZStickyPageControllerDelegate> delegate = (id<SYZStickyPageControllerDelegate>)self.delegate;
...
- (SYZMagicScrollView *)contentView {
    if (!_contentView) {
        _contentView = [SYZMagicScrollView new];
